...
contract tradeOffer {
        ...
        if (offerMapping[_offerId].offerAmount1 > 0) {
            require(
                IERC20(token1).transferFrom(
                    address(this),
                    msg.sender,
                    offerMapping[_offerId].offerAmount1
                ),
                "Transfer of token1 failed"
            );
        }
        ...
        if (offerMapping[_offerId].offerAmount2 > 0) {
            require(
                IERC20(token2).transferFrom(
                    address(this),
                    msg.sender,
                    offerMapping[_offerId].offerAmount2
                ),
                "Transfer of token2 failed"
            );
        }
        ...
        if (offerMapping[_offerId].offerAmount3 > 0) {
            require(
                IERC20(token3).transferFrom(
                    address(this),
                    msg.sender,
                    offerMapping[_offerId].offerAmount3
                ),
                "Transfer of token3 failed"
            );
        }
        ...
        if (offerMapping[_offerId].offerAmount4 > 0) {
            require(
                IERC20(token4).transferFrom(
                    address(this),
                    msg.sender,
                    offerMapping[_offerId].offerAmount4
                ),
                "Transfer of token4 failed"
            );
        }
        ...
        if (offerMapping[_offerId].offerAmount5 > 0) {
            require(
                IERC20(token5).transferFrom(
                    address(this),
                    msg.sender,
                    offerMapping[_offerId].offerAmount5
                ),
                "Transfer of token5 failed"
            );
        }
        ...
        if (offerMapping[_offerId].wantedAmount1 > 0) {
            require(
                IERC20(token1).transferFrom(
                    msg.sender,
                    offerCreator,
                    offerMapping[_offerId].wantedAmount1
                ),
                "Transfer of token1 failed"
            );
        }
        ...
        if (offerMapping[_offerId].wantedAmount2 > 0) {
            require(
                IERC20(token2).transferFrom(
                    msg.sender,
                    offerCreator,
                    offerMapping[_offerId].wantedAmount2
                ),
                "Transfer of token2 failed"
            );
        }
        ...
        if (offerMapping[_offerId].wantedAmount3 > 0) {
            require(
                IERC20(token3).transferFrom(
                    msg.sender,
                    offerCreator,
                    offerMapping[_offerId].wantedAmount3
                ),
                "Transfer of token3 failed"
            );
        }
        ...
        if (offerMapping[_offerId].wantedAmount4 > 0) {
            require(
                IERC20(token4).transferFrom(
                    msg.sender,
                    offerCreator,
                    offerMapping[_offerId].wantedAmount4
                ),
                "Transfer of token4 failed"
            );
        }
        ...
        if (offerMapping[_offerId].wantedAmount5 > 0) {
            require(
                IERC20(token5).transferFrom(
                    msg.sender,
                    offerCreator,
                    offerMapping[_offerId].wantedAmount5
                ),
                "Transfer of token5 failed"
            );
        }
        ...
        )
    {
        ...
        uint256 balance4 = IERC20(token4).balanceOf(address(this));
        uint256 balance5 = IERC20(token5).balanceOf(address(this));
    ...
    //Add offered tokens to the offer string
        ...
        if (offerAmount1 > 0) {
            abi.encodePacked(offer, uint256ToString(offerAmount1), " Clay ");
        }

        //concatenate offer string with Fish offered if any
        if (offerAmount2 > 0) {
            abi.encodePacked(offer, uint256ToString(offerAmount2), " Fish ");
        }

        //concatenate offer string with Rock offered if any
        if (offerAmount3 > 0) {
            abi.encodePacked(offer, uint256ToString(offerAmount3), " Rock ");
        }

        //concatenate offer string with Wood offered if any
        if (offerAmount4> 0) {
            abi.encodePacked(offer, uint256ToString(offerAmount4), " Wood ");
        }
        //concatenate offer string with Wool offered if any
        if (offerAmount5 > 0) {
            abi.encodePacked(offer, uint256ToString(offerAmount5), " Wool ");
        }

        abi.encodePacked(offer, " for ");

    //Add Wanted tokens to the offer string
        //concatenate offer string with Clay wanted if any
        if (wantedAmount1 > 0) {
            abi.encodePacked(offer, uint256ToString(wantedAmount1), " Clay ");
        }

        //concatenate offer string with Fish offered if any
        if (wantedAmount2 > 0) {
            abi.encodePacked(offer, uint256ToString(wantedAmount2), " Fish ");
        }

        //concatenate offer string with Rock offered if any
        if (wantedAmount3 > 0) {
            abi.encodePacked(offer, uint256ToString(wantedAmount3), " Rock ");
        }

        //concatenate offer string with Wood offered if any
        if (wantedAmount4> 0) {
            abi.encodePacked(offer, uint256ToString(wantedAmount4), " Wood ");
        }
        //concatenate offer string with Wool offered if any
        if (wantedAmount5 > 0) {
            abi.encodePacked(offer, uint256ToString(wantedAmount5), " Wool ");
        }
